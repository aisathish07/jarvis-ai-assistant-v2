# web_agent.py - FIXED: Robust web automation for MSI Thin 15 B13UC
...
# ========== FAST HTTP-ONLY SEARCH (No Browser) ==========
async def quick_search_http(query: str, max_results: int = 5) -> List[Dict[str, str]]:
    """
    Ultra-fast HTTP-only search (no browser overhead).
    Falls back to DuckDuckGo Lite - no JavaScript required.
    """
    # DuckDuckGo Lite - pure HTML, no anti-bot
    ...
    try:
        ...
        # DuckDuckGo Lite structure: <tr> with <a class="result-link">
...
async def example_usage():
    """How to use the fixed WebAgent."""
    agent = WebAgent()
    
    try:
        # Fast HTTP search (no browser)
        results = await agent.search("python programming", num_results=3)
        for r in results:
            print(f"- {r['title']}: {r['url']}")
        
        # Amazon search (uses browser)
        products = await agent.search_amazon("laptop")
        print(f"\nFound {len(products.get('products', []))} products")
        
        # Screenshot
        path = await agent.take_screenshot("https://example.com")
        print(f"\nScreenshot: {path}")
        
    finally:
        # IMPORTANT: Always close properly
        await safe_close_web_agent(agent)
